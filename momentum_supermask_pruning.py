...
class Supermask:
    def __init__(self, *args, **kwargs):
        ...
        self.conv_window = kwargs.pop("conv_window", None)
        self.fc_window = kwargs.pop("fc_window", None)
...
class SupermaskConv(Supermask, nn.Conv2d):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.scores_window = [torch.zeros_like(self.scores).cuda()] * self.conv_window # to(self.scores.device)
...
class SupermaskLinear(Supermask, nn.Linear):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.scores_window = [torch.zeros_like(self.scores).cuda()] * self.fc_window # to(self.scores.device)
...
def train(model, log_interval, device, train_loader, optimizer, criterion, epoch, penalty=0):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        model.conv1.scores_window.append(model.conv1.scores.grad)
        model.conv2.scores_window.append(model.conv2.scores.grad)
        model.fc1.scores_window.append(model.fc1.scores.grad)
        model.fc2.scores_window.append(model.fc2.scores.grad)
        model.fc3.scores_window.append(model.fc3.scores.grad)
        
        with torch.no_grad():
            model.conv1.scores -= model.conv1.scores_window.pop(0)
            model.conv2.scores -= model.conv2.scores_window.pop(0)
            model.fc1.scores -= model.fc1.scores_window.pop(0)
            model.fc2.scores -= model.fc2.scores_window.pop(0)
            model.fc3.scores -= model.fc3.scores_window.pop(0)
